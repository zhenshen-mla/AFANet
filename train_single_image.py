...
from models.image_single import make_network
...
writer = SummaryWriter(comment='Single')
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            if accuracy_gender > best_pred_gender:
                ...
                torch.save(model.state_dict(), path + 'Single_gender.pkl')
                print('save the gender ' + str(epoch) + 'model, replace the previous model')

            if accuracy_age > best_pred_age:
                best_pred_age = accuracy_age
                torch.save(model.state_dict(), path + 'Single_age.pkl')
                print('save the age ' + str(epoch) + 'model, replace the previous model')
